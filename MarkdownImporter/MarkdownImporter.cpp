...
static QLatin1StringView lfMarkerString () noexcept
{
    return QLatin1String ("---\n");
}
static QLatin1StringView crlfMarkerString () noexcept
{
    return QLatin1String ("---\r\n");
}

static const int qtmi_BlockQuoteIndent = 40;  // pixels, same as in QTextHtmlParserNode::initializeProperties

static_assert (int (MarkdownImporter::FeatureCollapseWhitespace) == MD_FLAG_COLLAPSEWHITESPACE);
static_assert (int (MarkdownImporter::FeaturePermissiveATXHeaders) == MD_FLAG_PERMISSIVEATXHEADERS);
static_assert (int (MarkdownImporter::FeaturePermissiveURLAutoLinks) == MD_FLAG_PERMISSIVEURLAUTOLINKS);
static_assert (int (MarkdownImporter::FeaturePermissiveMailAutoLinks) == MD_FLAG_PERMISSIVEEMAILAUTOLINKS);
static_assert (int (MarkdownImporter::FeatureNoIndentedCodeBlocks) == MD_FLAG_NOINDENTEDCODEBLOCKS);
static_assert (int (MarkdownImporter::FeatureNoHTMLBlocks) == MD_FLAG_NOHTMLBLOCKS);
static_assert (int (MarkdownImporter::FeatureNoHTMLSpans) == MD_FLAG_NOHTMLSPANS);
static_assert (int (MarkdownImporter::FeatureTables) == MD_FLAG_TABLES);
static_assert (int (MarkdownImporter::FeatureStrikeThrough) == MD_FLAG_STRIKETHROUGH);
static_assert (int (MarkdownImporter::FeatureUnderline) == MD_FLAG_UNDERLINE);
static_assert (int (MarkdownImporter::FeaturePermissiveWWWAutoLinks) == MD_FLAG_PERMISSIVEWWWAUTOLINKS);
static_assert (int (MarkdownImporter::FeaturePermissiveAutoLinks) == MD_FLAG_PERMISSIVEAUTOLINKS);
static_assert (int (MarkdownImporter::FeatureTasklists) == MD_FLAG_TASKLISTS);
static_assert (int (MarkdownImporter::FeatureNoHTML) == MD_FLAG_NOHTML);
static_assert (int (MarkdownImporter::DialectCommonMark) == MD_DIALECT_COMMONMARK);
...
{
    MarkdownImporter* mdi = static_cast<MarkdownImporter*> (userdata);
...
MarkdownImporter::MarkdownImporter (QTextDocument* doc, MarkdownImporter::Features features) : m_cursor (doc), m_monoFont (QFontDatabase::systemFont (QFontDatabase::FixedFont)), m_features (features)
{
}

MarkdownImporter::MarkdownImporter (QTextDocument* doc, QTextDocument::MarkdownFeatures features) : MarkdownImporter (doc, static_cast<MarkdownImporter::Features> (int (features)))
{
}

void MarkdownImporter::import (const QString& markdown)
{
    MD_PARSER callbacks = {
        0,  // abi_version
        unsigned (m_features), &CbEnterBlock, &CbLeaveBlock, &CbEnterSpan, &CbLeaveSpan, &CbText, &CbDebugLog,
        nullptr  // syntax
    };
    ...
    qDebug () << "default font" << defaultFont << "mono font" << m_monoFont;
...
int MarkdownImporter::cbEnterBlock (int blockType, void* det)
{
    ...
    case MD_BLOCK_P:
        ...
        {
            qDebug (m_listItem ? "P of LI at level %d" : "P continuation inside LI at level %d", int (m_listStack.size ()));
        }
        else
        {
            qDebug ("P");
    ...
    case MD_BLOCK_QUOTE:
        ++m_blockQuoteDepth;
        qDebug ("QUOTE level %d", m_blockQuoteDepth);
        break;
    ...
    case MD_BLOCK_CODE:
        {
            ...
            {
                qDebug ("CODE lang '%s' info '%s' fenced with '%c' inside QUOTE %d", qPrintable (m_blockCodeLanguage), qPrintable (info), m_blockCodeFence, m_blockQuoteDepth);
            }
            else
            {
                qDebug ("CODE lang '%s' info '%s' fenced with '%c'", qPrintable (m_blockCodeLanguage), qPrintable (info), m_blockCodeFence);
            }
        }
        break;
    ...
    case MD_BLOCK_H:
        {
            ...
            if (detail->level == 2)
            {
                charFmt.setForeground(QBrush (QColor (250, 100, 100)));
            }
    ...
    case MD_BLOCK_LI:
        {
            ...
            m_markerType = detail->is_task ? (detail->task_mark == ' ' ? QTextBlockFormat::MarkerType::Unchecked : QTextBlockFormat::MarkerType::Checked) : QTextBlockFormat::MarkerType::NoMarker;
            qDebug () << "LI";
        }
        break;
    ...
    case MD_BLOCK_UL:
        {
            ...
            qDebug ("UL %c level %d", detail->mark, int (m_listStack.size ()) + 1);
        }
        break;
    ...
    case MD_BLOCK_OL:
        {
            ...
            // TODO: compiler error m_listFormat.setStart (detail->start);
            qDebug ("OL xx%d level %d start %d", detail->mark_delimiter, int (m_listStack.size ()) + 1, detail->start);
        }
        break;
    ...
    case MD_BLOCK_TD:
        {
            ...
            auto fmt2 = cell.format().toTableCellFormat();
            fmt2.setBorderStyle(QTextFrameFormat::BorderStyle_None);
            fmt2.setLeftPadding (5);
            fmt2.setRightPadding (5);
            fmt2.setBottomPadding (5);
            fmt2.setTopPadding (5);
            cell.setFormat (fmt2);
            qDebug () << "TD; align" << detail->align << MdAlignment (detail->align) << "col" << m_tableCol;
        }
        break;
    ...
    case MD_BLOCK_TH:
        {
            ...
            auto fmt2 = fmt.toTableCellFormat();
            fmt2.setBorderStyle(QTextFrameFormat::BorderStyle_None);
            fmt2.setLeftPadding (5);
            fmt2.setRightPadding (5);
            fmt2.setBottomPadding (5);
    ...
    case MD_BLOCK_TABLE:
        {
            ...
            fmt.setBorder(1);
            fmt.setBorderBrush (QBrush (QColor (200, 200, 200)));
            fmt.setTopMargin(10);
            fmt.setBottomMargin(5);
            m_currentTable = m_cursor.insertTable (1, 1, fmt);  // we don't know the dimensions yet
        }
        break;
    case MD_BLOCK_HR:
        {
...
int MarkdownImporter::cbLeaveBlock (int blockType, void* detail)
{
    Q_UNUSED (detail)
    switch (blockType)
    {
    ...
    case MD_BLOCK_OL:
        ...
        {
            qWarning ("list ended unexpectedly");
        }
        else
        {
            qDebug ("list at level %d ended", int (m_listStack.size ()));
            m_listStack.pop ();
        }
        break;
    case MD_BLOCK_TR:
        {
            ...
            for (int col = m_tableCol; col >= 0; --col)
            {
                if (m_nonEmptyTableCells.contains (col))
                {
                    ...
                    {
                        qDebug () << "merging cells" << mergeBegin << "to" << mergeEnd << "inclusive, on row" << m_currentTable->rows () - 1;
    ...
    case MD_BLOCK_QUOTE:
        {
            qDebug ("QUOTE level %d ended", m_blockQuoteDepth);
            --m_blockQuoteDepth;
            m_needsInsertBlock = true;
        }
        break;
    case MD_BLOCK_TABLE:
        qDebug () << "table ended with" << m_currentTable->columns () << "cols and" << m_currentTable->rows () << "rows";
        m_currentTable = nullptr;
        m_cursor.movePosition (QTextCursor::End);
        break;
    case MD_BLOCK_LI:
        qDebug ("LI at level %d ended", int (m_listStack.size ()));
        m_listItem = false;
        break;
    case MD_BLOCK_CODE:
        {
            ...
            if (m_blockQuoteDepth)
            {
                qDebug ("CODE ended inside QUOTE %d", m_blockQuoteDepth);
            }
            else
            {
                qDebug ("CODE ended");
...
int MarkdownImporter::cbEnterSpan (int spanType, void* det)
{
    QTextCharFormat charFmt;
    ...
    qDebug () << spanType << "setCharFormat" << charFmt.font ().families ().constFirst () << charFmt.fontWeight () << (charFmt.fontItalic () ? "italic" : "") << charFmt.foreground ().color ().name ();
    m_cursor.setCharFormat (charFmt);
    return 0;  // no error
}

int MarkdownImporter::cbLeaveSpan (int spanType, void* detail)
{
    Q_UNUSED (detail)
    QTextCharFormat charFmt;
    if (!m_spanFormatStack.isEmpty ())
...
int MarkdownImporter::cbText (int textType, const char* text, unsigned size)
{
    ...
    if (m_imageSpan)
    {
        ...
        qDebug () << "image" << m_imageFormat.name () << "title" << m_imageFormat.stringProperty (QTextFormat::ImageTitle) << "alt" << s << "relative to" << m_cursor.document ()->baseUrl ();
    ...
    if (false)
    {
        ...
        if (m_cursor.currentList ())
        {
            debugInfo = "in list at depth " + QString::number (m_cursor.currentList ()->format ().indent ());
        }
        if (bfmt.hasProperty (QTextFormat::BlockQuoteLevel))
        {
            debugInfo += "in blockquote at depth " + QString::number (bfmt.intProperty (QTextFormat::BlockQuoteLevel));
        }
        if (bfmt.hasProperty (QTextFormat::BlockCodeLanguage))
        {
            debugInfo += "in a code block";
        }
        qDebug () << textType << "in block" << m_blockType << s << qPrintable (debugInfo) << "bindent" << bfmt.indent () << "tindent" << bfmt.textIndent () << "margins" << bfmt.leftMargin () << bfmt.topMargin () << bfmt.bottomMargin ()
                  << bfmt.rightMargin ();
    }
    return 0;  // no error
}
...
*/
void MarkdownImporter::insertBlock ()
{
